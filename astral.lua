...
local Chat = loadstring(game:HttpGet("https://raw.githubusercontent.com/Mastadawn/AstralV2/main/chatthingy.lua", true))()
...
if not isfile("Astral/Scripts.lua") then
	writefile("Astral/Scripts.lua",scriptExample)
end
...
if workspace:FindFirstChild"Map" then
	...
	Bedwars = loadstring(game:HttpGet("https://raw.githubusercontent.com/Mastadawn/AstralV2/main/Bedwars.lua", true))()
...
if game.Players.LocalPlayer.Character.PrimaryPart.Position.Y > 140 and not game.Workspace:FindFirstChild("spawn_cage") then
	Notifications.Notify("Astral","Script will load when game started",3.45,HUDColor)
end
...
local file = readfile("Astral/Scripts.lua")
if workspace:FindFirstChild("Map") then
    loadstring(file)()
end
...
UILibrary.MakeModule{
	...
	Function = function(Module, Dropdowns)
		...
		repeat
			task.spawn(function()
				if Utilities.IsAlive(game.Players.LocalPlayer.Character) then
					local Nearby = Utilities.GetNearbyPlayers(17.8, false)
					for i, v in pairs(Nearby) do
							...
							KillauraRemote:FireServer{
								...
								["validate"] = {
									["raycast"] = {
										...
										["selfPosition"] = {
											["value"] = getcloserpos(game.Players.LocalPlayer.Character.PrimaryPart.Position, Target.Character.PrimaryPart.Position, 5)
										},
										["targetPosition"] = {
